    ...
    if ($('#ddProvince').length) {
        ...
        $('#ddCountry').change(function() {
            $.ajax({
                ...
                data: "{'languageId': '" + $("#hdnDealerLocatorLanguageId").val() + "', 'countryGuid': '" + $('#ddCountry').val() + "'}",
                dataType: "json",
                success: function(result) {
                    var obj = $("#ddProvince");
                    $(obj).empty();
        ...
        if ($('#ddDistrict').length) {
            $('#ddProvince').change(function() {
                $.ajax({
                    ...
                    data: "{'languageId': '" + $("#hdnDealerLocatorLanguageId").val() + "', 'provinceGuid': '" + $('#ddProvince').val() + "'}",
                    dataType: "json",
                    success: function(result) {
                        var obj = $("#ddDistrict");
                        $(obj).empty();
        ...
        geocoder.geocode({ 'address': address, 'region': region }, function(results, status) {
            ...
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                $.ajax({
                    ...
                    //data: "distance=" + distance + "&lat=" + lat + "&lng=" + lng + "&languageid=" + $('#hdnDealerLocatorLanguageId').val() + "&siteid=" + $('#hdnDealerLocatorSiteId').val() + "&province=" + $('#ddProvince').val() + "&zoneguid=" + $('#hdnDealerLocatorZoneGuid').val() + "&district=" + $('#ddDistrict').val(),
                    data: { 
                        distance: distance,
                        lat: lat,
                        lng: lng,
                        languageid: $('#hdnDealerLocatorLanguageId').val(),
                        centerID: $('#slCenter').val(),
                        siteid: $('#hdnDealerLocatorSiteId').val(),
                        zoneguid: $('#hdnDealerLocatorZoneGuid').val(),
                        country: $('#ddCountry').val(),
                        province: $('#ddProvince').val(),
                    ...
                    success: function(msg) {
                        ...
                        if (results.length > 0) {
                            ...
                            jQuery.each(results, function(k, v) {
                                ...
                                if (number > 9) {
                                    $("<li id='l_" + marker['__gm_id'] + "' class='clinic_list double-digit ' />")
                                      ...
                                      .html("<h3>" + lang[currCulture].name + v.getAttribute("name") + "</h3><p>" + v.getAttribute("address") + "</p><p>" + phoneFax + "</p>")
									  .click(function() {
                                ...
                                google.maps.event.addListener(marker, 'click', function() {
                                    $('#list .clinic_list').removeClass('mapchooser-active');
                                    $('#list').animate($("#l_" + marker['__gm_id'])[0].scrollIntoView(true));

                                    $('#l_' + marker['__gm_id']).addClass('mapchooser-active');
                                    toggleBounce(marker);
                                    infowindow.setContent(info_window_string);
                                    infowindow.open(map, marker);
                                });
                            ...
                            $('.clinic_list').click(function() {
                                ...
                                $('#list .clinic_list').removeClass('mapchooser-active');
                                $(this).addClass('mapchooser-active');
...
function gmap_province_lookup(distance, region, userlat, userlng) {
    ...
    $.ajax({
        ...
        data: {
            ...
            lng: userlng,
            languageid: $('#hdnDealerLocatorLanguageId').val(),
            siteid: $('#hdnDealerLocatorSiteId').val(),centerID: $('#slCenter').val(),
            zoneguid: parentZone,
            country: $('#ddCountry').val(),
            province: $('#ddProvince').val(),
            district: $('#ddDistrict').val()
